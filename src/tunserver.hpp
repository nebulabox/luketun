...
class tun_server_session : public tun_session_base,
 ...
 private:
  void handle_request() {
    ...
    decode_pkg(in_socket_, [this, self](tun_pkg pkg) { handle_command(pkg.cmd, pkg.body); });
  }
  ...
  void do_read_from_in() {
    auto self(shared_from_this());
    decode_pkg(in_socket_, [this, self](tun_pkg pkg) { do_write_to_out(pkg.body); });
  }
  ...
  void do_accept() {
    acceptor_.async_accept(in_socket_, [this](std::error_code ec) {
      if (!ec) {
        ...
        std::make_shared<tun_server_session>(io_context_, std::move(in_socket_))->start();
      }
